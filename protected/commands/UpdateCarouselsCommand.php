...
class UpdateCarouselsCommand extends CConsoleCommand {
	...
	public function actionIndex($id = null) {
		...
		else	$carousels = array(Carousel::model()->findByPk($id));

		/** @var $carousel Carousel */
		foreach ($carousels as $carousel) {
			if (!($carousel instanceof Carousel)) throw new CException('Can\'t find carousel');

			if (!empty($carousel->client->logoUid)) $fs->resizeImage($carousel->client->logoUid, array($carousel->logoSize, $carousel->logoSize));
			...
			/** @var $item Item */
			foreach ($carousel->items as $item) $item->delete();

			foreach ($items as $itemAttributes) {
				$item = new Item();
				$item->setAttributes($itemAttributes);
				if (!$item->save()) throw new CException("Can't save Item:\n".print_r($item->getErrors(), true).print_r($item->getAttributes(), true));
			}
			$carousel->invalidate();

			if ($id !== null) {
				if ($allItemsCount > self::ITEMS_LIMIT)
					echo "Подлежат обработке ".$allItemsCount." записей. Из них случайно отобрано ".self::ITEMS_LIMIT.". Успешно обработано ".count($items).".";
				else	echo "Подлежат обработке ".$allItemsCount." записей. Успешно обработано ".count($items).".";
			}
		}
	}
}
